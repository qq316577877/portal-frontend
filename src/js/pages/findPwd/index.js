...
//		if($("#patterning").val()==""){
//			$(".img-false").css("display","inline-block").next().text("验证码不能为空");
//		}
//		$('#message-false').css('display','none');
//		httpRequest({
...
//					$('.img-false').next().text(response.msg);
...
//		if($('#tel').attr('pass') == 2) return false;
//		if($('#patterning').val().length < 4) return false;
//		if($('.input-pass').val().length < 6) return false;
//		httpRequest({
