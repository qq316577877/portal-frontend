...
function createDiv(zhuId, list) {
    ...
    var tagSum ='<input class="sumHidden" value="'+list.totalQuantity+'" style="display: none">';
    var tagSumDiv=document.createElement('div');
    tagSumDiv.innerHTML=tagSum;
    bigDiv.appendChild(tagSumDiv);
    head.className = 'pro-list-head';
    var tag2 =
        '<h3>货柜批次号：' + list.batchNumber + '</h3>' +
        '<h3 class="container-name" transactionNo="'+list.transactionNo+'">货柜名称：' + list.productName + '</h3>' +
        '<h3 class="container-norms">货柜规格：0-' + list.maxQuantity + '箱</h3>' ;
    head.innerHTML=tag2;
    ...
    var tag4 =    '<span>总计：</span>'+
        '<input type="text" value="'+list.productAmount+'" disabled/>'+
        '<span>元</span>';
